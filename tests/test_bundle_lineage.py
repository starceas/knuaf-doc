...
class RealTreeTests(ContractCase):
    def test_candidate_verifies_under_v2(self):
        ...
        if complete:
            ...
            # P16 (industrial_insects second bundle) updates declared
            # runtime files on top of P15.
            self.assertEqual(113, rc["verified"])
            self.assertEqual(86, rc["new_count"])
        else:
    ...
    def test_manifest_v2_link(self):
        ...
        self.assertEqual("29abec0ec95369de7ae9e22207349d574f1f46b7",
                         link["baseline_main"])
        # P16's parent is the accepted P15 tree (188 files).
        self.assertEqual(113, link["change_count"])
        self.assertEqual(86, link["new_count"])
        self.assertEqual(86, link["approved_new_files"])
        ...
        # The P15 D5 correction record keeps its own parent (the P14
        # tree); that tree stays in the ancestor list as P14.
        d5 = json.loads((REPO_ROOT / "docs/validation/d5-rda-useful-life.json")
                        .read_text(encoding="utf-8"))
        self.assertIn(("P14", d5["parent_tree_sha256"]),
                      [(row["stage"], row["tree_sha256"]) for row in lineage])
