...
class RealTreeTests(ContractCase):
    def test_candidate_verifies_under_v2(self):
        ...
        if complete:
            ...
            # P16 (rda-lookup verification status; provisional stage
            # number, rebased onto accepted P15) declares one new
            # test file on top of P15's 113 declared changes.
    ...
    def test_manifest_v2_link(self):
        ...
        self.assertEqual(
            ...
            link["spec_sha256"])
        self.assertEqual("P16", link["stage"])
        self.assertEqual("29abec0ec95369de7ae9e22207349d574f1f46b7",
                         link["baseline_main"])
        # P16's parent is the accepted P15 tree (188 files, main
        # b489147d — PR #13 D5 merge).  Provisional number: the
        # stage is renumbered P17 when PR #17 lands.
        self.assertEqual(114, link["change_count"])
        self.assertEqual(87, link["new_count"])
        self.assertEqual(87, link["approved_new_files"])
        self.assertEqual("P15", link["parent_candidate"]["stage"])
        self.assertEqual(188, link["parent_candidate"]["file_count"])
        self.assertEqual(
            "a0854e3347144a3c5ebd5ac38afd47c7845b2d1142807c3ad39d03aa2651f848",
            link["parent_candidate"]["tree_sha256"])
        # D5's receipt pins its own parent: the accepted P14 tree it
        # was rebaselined onto — under P16 that is lineage[-2], one
        # step before the P15 parent_candidate.
        d5 = json.loads((REPO_ROOT / "docs/validation/d5-rda-useful-life.json")
                        .read_text(encoding="utf-8"))
        self.assertEqual("P14", link["stage_lineage"][-2]["stage"])
        self.assertEqual(d5["parent_tree_sha256"],
                         link["stage_lineage"][-2]["tree_sha256"])
        # The ancestor list ends at the parent; the current stage is not
        # one of its own ancestors.
        lineage = link["stage_lineage"]
        self.assertEqual(
            {k: link["parent_candidate"][k]
             for k in ("stage", "file_count", "tree_sha256")},
            {k: lineage[-1][k] for k in ("stage", "file_count", "tree_sha256")})
        self.assertNotIn("P16", [row["stage"] for row in lineage])
        ...
        for name, meta in rc["changes"].items():
            ...
            if name in baseline:
                ...
                self.assertIn(meta["first_changed"],
                              {"P1", "P2", "P3", "P5", "P7", "P9", "P10", "P11", "P12", "P13", "P15", "P16"})
                self.assertIn(meta["owner"],
                              {"A", "B", "C", "P4", "D", "P6", "P7", "P8",
                               "P9", "P10", "P11", "P12", "P13", "P14", "P15", "P16"})
            else:
                # P2 through P16 new files: pre-declared allowlist only.
