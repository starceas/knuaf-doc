...
| id (`project.json` 또는 매핑 ID) | 자료 | 허용 역할 | 금지 |
|---|---|---|---|
| `official-writing-guide-pdf` | 2020 학교 공식 작성요령 PDF | **이 2020 판에서만 1–5쪽:** 작성 지침·판정 근거. **6쪽 이후:** 양식·예시. Q1b 해시 일치 시 재파싱 생략 | 페이지 역할을 섞어 규칙을 만들기, 원문에 없는 규칙 보충 |
| `official-writing-guide-hwp` | 2020 요령 HWP 짝 | PDF와 동일 원본의 한글 파일. 페이지 분할은 PDF를 따름 | PDF 분할을 HWP에 자동 적용, 재파싱 |
| `kim-wonseop-exemplar-pdf` | 김원섭 선배논문(교수 강의자료) 지황 PDF | 서술의 깊이·논증 연결·내러티브 품질 벤치마크. Q1b 해시 일치 시 128쪽 재파싱 생략 | 공식 규칙 우선순위 변경, 학생 사실·가격·연도·근거 복제 |
| `kim-wonseop-exemplar-hwp` | 김원섭 논문 HWP 짝 | PDF와 동일 원본 | PDF를 다시 OCR |
| `seo-minseo-finance-xlsx` | 서민서 재무 작성예시 XLSX (다른 작성자) | 재무 템플릿 1순위. 형식·수식·병합·시트명 보존. Q1b에서는 값 덤프 생략 | 김원섭 자료와 같은 작성자라고 추정, 값·학생 정보 복제 |
| `specialty-22160117-finance-xlsx` | 22160117 특용 졸업논문 엑셀 | 특용 재무 예시 2순위. Q1b 재파싱 생략. 템플릿 정본 아님 | 현재 계획 사실·학번 정보 승격, 서민서 시트맵 자동 적용 |
| `rda.income.national.2024` | 2024 농산물소득자료집 | 내장 팩 조회. 폴더의 원본 PDF는 스킵 | 웹 소득으로 덮어쓰기, 원본 재파싱 |
| `rda.income.regional.2024` | 2024 지역별 농산물소득자료 | 내장 팩 조회. 지역 사례이며 통계적 유의성 없음 | 전국 평균과 합치기, 원본 재파싱 |
| `rda.econ.2025` | 2025 경제성분석 기준자료집 | 내장 팩 조회 | 없는 시설 신축단가 표를 추정으로 채우기 |
| `mafra.specialty.production.2024` | 2024 특용작물 생산실적 | 특용 전공 팩 조회 | 인삼이 이 조사에 있다고 우기기, 원본 재파싱 |
| `kim-sunghak-farm-accounting-pptx` | 김성학 농수산경영회계 PPTX(142슬라이드) | Ⅳ장 해석 잣대. `kim-finance-bible.md` 발췌 인용만 | 원본 재파싱, 엑셀 재계산, 예시 숫자 복제 |
| `kim-sunghak-farm-finance-textbook-pdf` | 김성학 기업·농업회계 및 재무분석 교재 PDF(594쪽) | 비율·BEP·현가표 위치 인용만 | 594쪽 반입, K-IFRS 기업비율로 작성요령 덮기 |

공식 PDF와 교수 지시가 충돌하면 해당 페이지와 지시를 함께 기록하고 교수의 현재 지시를 우선한다. 이 2020 PDF의 1–5쪽 지침과 6쪽 이후 양식·예시는 각각 `authority`를 다르게 기록한다. 다른 연도·개정판 PDF에 이 페이지 분할을 자동 적용하지 않는다. 새 원본은 해시와 실제 페이지를 먼저 등록하고, 그 판의 규칙 위치를 다시 매핑한다. 김원섭 선배논문은 교수 강의자료라는 맥락을 기록하되 공식 규칙을 덮어쓰지 않는다. 서민서 XLSX는 템플릿이 제공되면 새 17시트를 처음부터 재조립하지 않고 복제한 뒤 명시된 입력 셀만 비운다. 22160117 엑셀은 같은 특용 예시이지만 템플릿 1순위가 아니며 값을 현재 계획으로 옮기지 않는다.

Q1b 폴더에서 위 ID의 **해시 일치본**이 보이면 [source-intake.md](source-intake.md) §0에 따라 본문 열람을 생략하고 인덱스에만 매핑한다. 파일명 유사만으로는 매핑하지 않는다. 기계 목록은 [builtin-sources.json](builtin-sources.json)이다. 스킵 원본을 `project.json.sources[].path`로 넣어 `apply()`가 데스크톱 파일을 작업폴더 해시 대상으로 만들게 하지 않는다. 인용은 내장 팩 locator·규칙 파일·이미 등록된 작업폴더 상대경로만 사용한다.
